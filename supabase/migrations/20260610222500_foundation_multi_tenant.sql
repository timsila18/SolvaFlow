...
as $$
begin
  if tg_table_name = 'customers' and (new.customer_code is null or new.customer_code = '') then
    new.customer_code := private.next_document_number(new.company_id, 'customer');
  elsif tg_table_name = 'suppliers' and (new.supplier_code is null or new.supplier_code = '') then
    new.supplier_code := private.next_document_number(new.company_id, 'supplier');
  elsif tg_table_name = 'warehouses' and (new.code is null or new.code = '') then
    new.code := private.next_document_number(new.company_id, 'warehouse');
  elsif tg_table_name = 'vehicles' and (new.vehicle_code is null or new.vehicle_code = '') then
    new.vehicle_code := private.next_document_number(new.company_id, 'vehicle');
  elsif tg_table_name = 'products' and (new.sku is null or new.sku = '') then
